...
def float_from_int16_bytes(b: bytes, channels: int) -> np.ndarray:
    ...
    if channels > 1: arr = arr.reshape(-1, channels)
...
def list_devices():
    ...
    try:
        try: wasapi_idx = p.get_host_api_info_by_type(pyaudio.paWASAPI)["index"]
        except Exception: wasapi_idx = None
        for i in range(p.get_device_count()):
            d = p.get_device_info_by_index(i)
            info = {"index": d["index"], "name": d["name"], "hostApi": d["hostApi"],
            ...
            if is_input: inputs[info["name"]] = info
            if not is_input or is_wasapi:
                if d["maxOutputChannels"] > 0 and not info["isLoopbackDevice"]: outputs[info["name"]] = info
                if info["isLoopbackDevice"]: loopbacks[info["name"]] = info
    finally: p.terminate()
...
class RecordingController(threading.Thread):
    ...
    def _find_supported(self, dev_idx, sr, ch, is_input):
        ...
        for sr_try in [sr] + SR_CANDIDATES:
            for ch_try in [ch, 2, 1]:
                try:
                    if self.p.is_format_supported(sr_try,
                        input_device=dev_idx if is_input else None, output_device=dev_idx if not is_input else None,
                        input_channels=ch_try if is_input else 0, output_channels=ch_try if not is_input else 0,
                        input_format=pyaudio.paInt16): return True, ch_try, sr_try
    ...
    def _open_mic(self, mic_dev):
        ...
        self.mic_stream = self.p.open(format=pyaudio.paInt16, channels=ch, rate=sr, input=True,
    ...
    def _open_loopback(self, lb_dev):
        ...
        try:
            self.spk_stream = self.p.open(format=pyaudio.paInt16, channels=ch, rate=sr, input=True,
    ...
    def start_recording(self, mic_device=None, spk_device=None, loopback_device=None, include_mic=True,
        ...
        self._ensure_pyaudio()
        if not output_path: return self.status_queue.put(("ERROR", "No output file selected."))
        if not spk_device and not loopback_device: return self.status_queue.put(("ERROR", "Select an output/loopback device."))
        if include_mic and not mic_device: return self.status_queue.put(("ERROR", "Select a microphone."))
        ...
        try:
            if loopback_device: lb_choice = loopback_device
            else:
            ...
            if self.include_mic: self._open_mic(mic_device)
            
            command = ['ffmpeg', '-y', '-f', 's16le', '-ar', str(TARGET_SR), '-ac', str(self.loop_channels),
            ...
            self.ffmpeg_process = subprocess.Popen(command, stdin=subprocess.PIPE, stdout=subprocess.DEVNULL,
    ...
    def stop_recording(self):
        ...
        for t in [self.mic_thread, self.loop_thread, self.processing_thread]:
            if t: t.join()
        try:
            if self.ffmpeg_process and self.ffmpeg_process.stdin: self.ffmpeg_process.stdin.close()
            if self.ffmpeg_process:
                _, stderr = self.ffmpeg_process.communicate(timeout=10)
                if self.ffmpeg_process.returncode != 0:
                    self.status_queue.put(("ERROR", f"FFmpeg error:\n{stderr.decode('utf-8','ignore')}"))
                else: self.status_queue.put(("INFO", f"Recording saved to\n{self.output_path}"))
        except Exception as e: self.status_queue.put(("ERROR", f"Error finalizing file: {e}"))
        finally: self.ffmpeg_process, self.output_path = None, None
        self._teardown()
    ...
    def _processing_loop(self):
        ...
        while not self.loop_queue.empty() or not self.mic_queue.empty():
            loop_raw = self._get_chunk_from_queue(self.loop_queue, BLOCK, self.loop_channels)
            mic_raw = self._get_chunk_from_queue(self.mic_queue, BLOCK, self.mic_channels) if self.include_mic else np.zeros((BLOCK, self.mic_channels), dtype=np.float32)
            self._process_and_write_chunk(loop_raw, mic_raw)
    ...
    def _process_and_write_chunk(self, loop_raw, mic_raw):
        ...
        if loop_rs.size > 0 and np.max(np.abs(loop_rs)) >= CLIP_LEVEL: self.status_queue.put(("CLIP", "sys"))
        if mic_rs.size > 0 and np.max(np.abs(mic_rs)) >= CLIP_LEVEL: self.status_queue.put(("CLIP", "mic"))
        ...
        if mixed.size > 0 and self.ffmpeg_process and self.ffmpeg_process.stdin:
            try: self.ffmpeg_process.stdin.write(to_int16(mixed).tobytes())
...
class AudioRecorderApp:
    MP3_QUALITY_PRESETS = {"Good (192kbps)": "192k", "Standard Voice (128kbps)": "128k",
                           "Low (96kbps)": "96k", "Lowest - Fast (64kbps)": "64k"}
    ...
    def _create_ui(self):
        main = tk.Frame(self.root, padx=15, pady=15); main.pack(expand=True, fill=tk.BOTH)
        top_row = tk.Frame(main); top_row.pack(fill=tk.X, expand=True, pady=(0, 10))
        top_row.grid_columnconfigure(0, weight=1); top_row.grid_columnconfigure(1, weight=1)
        ...
        self.mic_menu = tk.OptionMenu(devf, self.selected_mic_name, "..."); self.mic_menu.grid(row=1, column=0, sticky="ew", pady=(0, 5))
        tk.Label(devf, text="Audio Output (device to capture):").grid(row=2, column=0, sticky="w")
        self.spk_menu = tk.OptionMenu(devf, self.selected_spk_name, "..."); self.spk_menu.grid(row=3, column=0, sticky="ew", pady=(0, 5))
        tk.Label(devf, text="Loopback device (override):").grid(row=4, column=0, sticky="w")
        self.lb_menu = tk.OptionMenu(devf, self.selected_lb_name, "..."); self.lb_menu.grid(row=5, column=0, sticky="ew")
        tk.Checkbutton(devf, text="Include microphone in recording", variable=self.include_mic_var).grid(row=6, column=0, sticky="w", pady=(10, 0))
        ...
        self.browse_btn = tk.Button(outf, text="Browse...", command=self.browse_directory); self.browse_btn.grid(row=1, column=1, sticky="e")
        tk.Label(outf, text="MP3 Quality:").grid(row=2, column=0, sticky="w", pady=(8,0))
        tk.OptionMenu(outf, self.mp3_quality_var, *self.MP3_QUALITY_PRESETS.keys()).grid(row=3, column=0, columnspan=2, sticky="ew")
        outf.grid_columnconfigure(0, weight=1)
        bottom_row = tk.Frame(main); bottom_row.pack(fill=tk.X, expand=True, pady=10)
        bottom_row.grid_columnconfigure(0, weight=1); bottom_row.grid_columnconfigure(1, weight=1)
        ...
        self.refresh_btn = tk.Button(main, text="Refresh Devices", command=self.populate_device_lists); self.refresh_btn.pack(pady=5)
        self.status_label = tk.Label(main, text="Ready to record", font=("Arial", 12)); self.status_label.pack(pady=5)
        btnrow = tk.Frame(main); btnrow.pack(pady=6)
        self.start_btn = tk.Button(btnrow, text="Start Recording", command=self.start_clicked); self.start_btn.pack(side=tk.LEFT, padx=5)
        self.stop_btn = tk.Button(btnrow, text="Stop Recording", command=self.stop_clicked, state=tk.DISABLED); self.stop_btn.pack(side=tk.LEFT, padx=5)
    ...
    def _create_meter_row(self, parent, label_text):
        ...
        db_label = tk.Label(r, text="-inf dBFS", width=10, anchor="w"); db_label.pack(side=tk.LEFT)
        clip_label = tk.Label(r, text="CLIP", fg="lightgrey", font=("Arial", 8, "bold")); clip_label.pack(side=tk.LEFT, padx=5)
    ...
    def start_clicked(self):
        ...
        path = filedialog.asksaveasfilename(parent=self.root, initialdir=self.output_directory.get(), initialfile=fname,
    ...
    def stop_clicked(self): self.command_queue.put(("STOP", {}))
        ...
        try:
            while True:
                ...
                if msg == "STATUS":
                    ...
                    if "Ready" in data: self.start_btn.config(state=tk.NORMAL); self.stop_btn.config(state=tk.DISABLED)
                    ...
                    label.config(text=f"{fmt_dbfs(peak):.1f} dBFS")
                elif msg == "CLIP":
                    indicator, after_id = (self.mic_clip, self.mic_clip_after_id) if data == "mic" else (self.sys_clip, self.sys_clip_after_id)
                    if after_id: self.root.after_cancel(after_id)
                    indicator.config(fg="red")
                    new_id = self.root.after(1500, lambda ind=indicator: ind.config(fg="lightgrey"))
                    if data == "mic": self.mic_clip_after_id = new_id
                    else: self.sys_clip_after_id = new_id
        except queue.Empty: pass
        finally: self._poll_id = self.root.after(100, self.process_status)

    def _save_settings(self):
        settings = {"mic_device": self.selected_mic_name.get(), "spk_device": self.selected_spk_name.get(),
        ...
        try: CONFIG_FILE.write_text(json.dumps(settings, indent=4))
    ...
    def _load_settings(self):
        ...
        if not CONFIG_FILE.exists():
            ...
            self.output_directory.set(str(default_dir)); return
        try:
            ...
            if Path(saved_dir).is_dir(): self.output_directory.set(saved_dir)
            else:
    ...
    def populate_device_lists(self):
        ...
        def _populate(menu, var, devices, default):
            menu['menu'].delete(0, 'end')
            if devices:
                for name in devices: menu['menu'].add_command(label=name, command=lambda v=name: var.set(v))
                var.set(default if default in devices else next(iter(devices), ""))
