...
WIN_OUT="$(wslpath -w "/mnt/c/Users/radek/dev/hearbud")"

# Kill any running app (ignore errors). UNC warning is harmless.
cmd.exe /c "taskkill /IM Hearbud.exe /F >NUL 2>&1" || true

# Restore + publish (note: no ^ line continuations; bash uses backslashes, but we keep it single-line)
"$WIN_DOTNET" restore "$WIN_REPO_ROOT\\Hearbud.sln"
"$WIN_DOTNET" publish "$WIN_REPO_ROOT\\Hearbud\\Hearbud.csproj" -c Release -r win-x64 \
