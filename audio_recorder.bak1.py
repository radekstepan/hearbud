...
class RecordingController(threading.Thread):
    ...
    Compatibility: If PyAudio build doesn't support 'as_loopback', we retry without it,
    which works when the chosen device itself is a loopback input.
    """
    ...
    def _find_supported(self, device_index, suggested_sr, channels, is_input):
        """Probe the device for a workable samplerate/channels."""
    ...
    def _open_loopback_by_index(self, lb_dev):
        """Open a loopback device, with compatibility for PyAudio builds lacking 'as_loopback'."""
        lb_idx = lb_dev['index']
        ...
        for _ in range(5):
            ...
            try:
                ...
                # Compatibility: retry WITHOUT as_loopback if the device is already a loopback endpoint
                try:
                    self.spk_stream = self.p.open(
                        format=pyaudio.paInt16,
                        channels=ch,
        ...
        raise RuntimeError(
            ...
            f"Tip: In the app, pick an entry under 'Loopback device (what to capture)'. "
            f"If you only have 'outputs' listed and no 'loopback' entries, make sure your "
            f"audio driver exposes WASAPI loopback (Windows) or install pyaudiowpatch."
        )

    def _match_loopback(self, spk_dev, loopbacks: dict):
        """Find a loopback device corresponding to the selected speaker by name."""
        base = spk_dev['name'] if spk_dev else ""
        # Exact prefix match (e.g., "<Speaker name> (loopback)")
    ...
    def start_recording(self, mic_device=None, spk_device=None, loopback_device=None,
                        include_mic=True, raw_dump=False):
        self._ensure_pyaudio()
        ...
        # Open loopback: prefer explicit selection; else try auto-match
        try:
        ...
        if self.raw_dump:
            # Prepare raw WAV path (auto-saves, no dialog)
    ...
    def _read_loopback(self):
        ...
        try:
            while self.is_recording and self.spk_stream and self.spk_stream.is_active():
                ...
                # Some PyAudio builds expose _channels; fallback to the configured one
                ch = getattr(self.spk_stream, "_channels", None)
                if ch is None:
                    ch = 2
    ...
    def _read_mic(self):
        ...
        try:
            while self.is_recording and self.mic_stream and self.mic_stream.is_active():
                ...
                ch = getattr(self.mic_stream, "_channels", None)
                if ch is None:
                    ch = 1
                x = float_from_int16_bytes(data, ch)
    ...
    def _process_and_emit(self):
        ...
        if not mic_frames:
            ...
            loop_np *= LOOP_GAIN
            peak = float(np.max(np.abs(loop_np))) if loop_np.size else 0.0
            if peak > 1e-9 and peak > TARGET_PEAK:
                loop_np *= (TARGET_PEAK / peak)

            loop_rs = resample_to(loop_np, self.capture_sr_spk, TARGET_SR)
            peak2 = float(np.max(np.abs(loop_rs))) if loop_rs.size else 0.0
            if peak2 > 1e-9 and peak2 > TARGET_PEAK:
                loop_rs *= (TARGET_PEAK / peak2)

            mixed_i16 = to_int16(loop_rs)
            self.status_queue.put(("INFO", f"System-only peak: {peak:.3f} (saved @ {TARGET_SR} Hz)"))
        ...
        if not loop_frames:
            ...
            peak = float(np.max(np.abs(mic_rs))) if mic_rs.size else 0.0
            if peak > TARGET_PEAK:
                mic_rs *= (TARGET_PEAK / peak)
        ...
        self.status_queue.put(("INFO", f"Mic peak: {float(np.max(np.abs(mic_np))):.3f} | "
                                       f"System peak: {float(np.max(np.abs(loop_np))):.3f} | Delay: 0 samples"))
        ...
        if SAVE_STEMS:
            try:
                ...
                ts = datetime.now().strftime("%Y-%m-%d_%H-%M-%S")
                write_wav(str(dbg / f"{ts}-mic.wav"), TARGET_SR, to_int16(mic2))
                write_wav(str(dbg / f"{ts}-loop.wav"), TARGET_SR, to_int16(loop2))
...
class AudioRecorderApp:
    def __init__(self, root):
        ...
        tk.Label(devf, text=tip, wraplength=580, justify=tk.LEFT, fg="darkblue")\
        ...
        dbg_btn = tk.Button(devf, text="Show device debug list", command=self.show_device_debug)
        dbg_btn.grid(row=9, column=0, sticky="w", pady=(8,0))

        devf.grid_columnconfigure(0, weight=1)
        devf.grid_columnconfigure(1, weight=1)
        devf.grid_columnconfigure(2, weight=1)
    ...
    def populate_device_lists(self):
        ...
        # keep selection if still valid; else default to Auto
    ...
    def show_device_debug(self):
        ins, outs, lbs = self.mic_devices, self.spk_devices, self.lb_devices
        lines = ["WASAPI devices seen by PyAudio:", "",
                 "=== Microphones / Inputs ==="]
        lines += [fmt_dev_line(d) for d in ins.values()] or ["(none)"]
        lines += ["", "=== Render Outputs ==="]
        lines += [fmt_dev_line(d) for d in outs.values()] or ["(none)"]
        lines += ["", "=== Loopback Inputs (capture these) ==="]
        lines += [fmt_dev_line(d) for d in lbs.values()] or ["(none)"]
        with self.modal_guard():
            messagebox.showinfo("Device Debug", "\n".join(lines))
